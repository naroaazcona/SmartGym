...
const renderEmptyState = () => `
  <div class="empty-state" style="grid-column:1/-1">
    <div class="empty-icon">
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 10c-.83 0-1.5-.67-1.5-1.5v-5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5v5c0 .83-.67 1.5-1.5 1.5z"/><path d="M20.5 10H19V8.5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/><path d="M9.5 14c.83 0 1.5.67 1.5 1.5v5c0 .83-.67 1.5-1.5 1.5S8 21.33 8 20.5v-5c0-.83.67-1.5 1.5-1.5z"/><path d="M3.5 14H5v1.5c0 .83-.67 1.5-1.5 1.5S2 16.33 2 15.5 2.67 14 3.5 14z"/><path d="M14 14.5c0-.83.67-1.5 1.5-1.5h5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5h-5c-.83 0-1.5-.67-1.5-1.5z"/><path d="M15.5 19H14v1.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5-.67-1.5-1.5-1.5z"/><path d="M10 9.5C10 8.67 9.33 8 8.5 8h-5C2.67 8 2 8.67 2 9.5S2.67 11 3.5 11h5c.83 0 1.5-.67 1.5-1.5z"/><path d="M8.5 5H10V3.5C10 2.67 9.33 2 8.5 2S7 2.67 7 3.5 7.67 5 8.5 5z"/></svg>
    </div>
...
const sortClassesByDate = (items = []) =>
  items
    .filter((item) => {
      const startsAt = asDate(item?.starts_at);
...
const renderMemberClassCard = (cls, options = {}) => {
  ...
  const startsAt = asDate(cls?.starts_at);
  const isBookable = startsAt ? startsAt.getTime() > Date.now() : false;
  ...
  const progressColor =
    occupancy >= 85 ? "rgba(239,68,68,.80)" : occupancy >= 60 ? "rgba(245,158,11,.80)" : "rgba(16,185,129,.80)";
  ...
  return `
    <article class="class-card member-class-card ${demand.className}" data-class-id="${cls.id}" data-demand="${demand.key}">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:8px; flex-wrap:wrap;">
        <div class="tag">${escapeHtml(cls.class_type_name || "Clase")}</div>
        <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
          <span class="badge">${occupancy}% aforo</span>
          ${isReserved ? `<span class="badge">Reservada ✓</span>` : ""}
        </div>
      </div>

      <div style="display:flex; justify-content:space-between; gap:12px; align-items:flex-start;">
        <div>
          <div style="font-weight:600; font-size:16px;">${escapeHtml(formatClassDate(cls.starts_at))}</div>
    ...
    cls.instructor_name ? `Coach ${escapeHtml(cls.instructor_name)}` : "Coach por asignar"
      ...
      <div style="display:flex; flex-direction:column; gap:6px; margin-top:4px;">
        <div style="display:flex; justify-content:space-between; gap:8px; align-items:center;">
          <div class="dim">Aforo ${booked}/${capacity}</div>
          <span class="member-demand-badge ${demand.className}">${demand.label}</span>
        </div>
        <div style="height:5px; width:100%; border-radius:999px; background:rgba(24,24,27,.08); overflow:hidden;">
      ...
      <div class="cta-inline" style="margin-top:auto;">
        <button class="btn btn-primary" data-action="reserve" data-id="${cls.id}" ${full || isReserved || !isBookable ? "disabled" : ""}>
          ${isReserved ? "Reservada" : isBookable ? "Reservar" : "Cerrada"}
        </button>
          ...
          <div class="card" style="display:flex; flex-direction:column; gap:16px;">
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:10px;">
              <div class="card" style="padding:14px; box-shadow:none; background:var(--surface-2); border-color:var(--border);">
                ...
                <div style="font-size:20px; font-weight:700;" id="member-visible-count">${sortedClasses.length} clases</div>
              </div>
              <div class="card" style="padding:14px; box-shadow:none; background:var(--surface-2); border-color:var(--border);">
                <div class="kicker">Próxima clase</div>
                <div style="font-size:14px; font-weight:500;" id="member-next-class">${sortedClasses[0] ? escapeHtml(formatClassDate(sortedClasses[0].starts_at)) : "Sin clases"}</div>
              </div>
              <div class="card" style="padding:14px; box-shadow:none; background:var(--surface-2); border-color:var(--border);">
                <div class="kicker">Plazas hoy</div>
                <div style="font-size:20px; font-weight:700;" id="member-free-today">--</div>
              </div>
            </div>
